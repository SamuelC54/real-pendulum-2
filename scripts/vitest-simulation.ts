...
const PHYSICS_SIM_DIR = path.resolve(__dirname, "../apps/simulation");
/** Isolated port so dev simulation on 58871 does not shadow test server code. */
...
async function waitForReady(timeoutMs: number): Promise<void> {
  ...
  throw new Error(
    "simulation did not become ready. Install deps: pip install -r apps/simulation/requirements.txt",
  );
}
...
export async function setup(): Promise<void> {
  ...
  proc.on("error", (err) => {
    console.error("[vitest-simulation] failed to spawn:", err.message);
  });
