import { config, coupledSimGrpcBaseUrl } from "@real-pendulum/app-config";
import * as motor from "@real-pendulum/motor-service/sdk";
import * as sensor from "@real-pendulum/sensor-service/sdk";

/** Default coupled sim (motor + sensor on one host), when sim URLs are unset. */
export function defaultCoupledSimGrpcUrl(): string {
  return coupledSimGrpcBaseUrl();
}
