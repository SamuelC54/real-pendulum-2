...
CMake builds the Teknic **`teknic_motor`** shared library. **`scripts/build-native.mjs`** (run via **`npm run build:native`** on **`@real-pendulum/physical-motor-service`**) configures **`CMakeLists.txt`** in this directory.
...
If CMake was ever configured with extra toolchains, delete **`build/`** under this folder and run **`npm run build:native -w @real-pendulum/physical-motor-service`** again. The entire **`build/`** directory is gitignored.
